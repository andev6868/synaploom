...
func newFilesystemActivityCatalog(courseID, version string, sources map[string][]course.ActivitySetSource) (*filesystemActivityCatalog, error) {
	catalog := &filesystemActivityCatalog{owners: make(map[activity.OwnerIdentity]activityCatalogOwner, len(sources))}
	for lessonID, activitySets := range sources {
		ownerID := activity.OwnerIdentity{CourseID: courseID, CourseVersion: version, Kind: activity.OwnerKindLesson, ID: lessonID}
		...
		for _, source := range activitySets {
			...
			for _, activitySource := range source.Activities {
				...
				if err != nil {
					return nil, fmt.Errorf("decode activity %s: %w", activitySource.ID, err)
				}
