...
const policy: ActivitySetPolicy = {
  purpose: 'practice', maxAttempts: null, feedbackMode: 'immediate', revealAnswers: 'after-submit', scoring: 'points', passingScore: null,
};
...
function attempt(status: ActivityAttempt['status'], revision = 1): ActivityAttempt {
  return {
    id: 'attempt', courseId: 'course', courseVersion: '1', ownerKind: 'lesson', ownerId: 'lesson', activityId: 'quiz', attemptNumber: 1,
    status, answer, score: status === 'EVALUATED' ? 1 : null, maxScore: status === 'EVALUATED' ? 1 : null,
    passed: status === 'EVALUATED' ? true : null,
    feedback: status === 'EVALUATED' ? { summary: 'Đúng', details: [], nextAction: 'continue' } : null,
    startedAt: '2026-07-18T00:00:00Z', submittedAt: status === 'DRAFT' ? null : '2026-07-18T00:00:01Z', evaluatedAt: status === 'EVALUATED' ? '2026-07-18T00:00:02Z' : null,
    revision,
  };
}
...
function api(overrides: Partial<SynaploomApiClient> = {}): SynaploomApiClient {
  ...
  return {
    getCourse: unused, getNavigation: unused, getLessonView: unused, getChapterAssessment: unused, recordChapterAssessment: unused,
    getActivity: unused, getCurrentActivityAttempt: () => Promise.resolve(null),
    saveActivityDraft: () => Promise.resolve(attempt('DRAFT')),
    submitActivityAttempt: () => Promise.resolve(attempt('EVALUATED')),
    getActivitySetProgress: unused, getCurrentLesson: unused, getLesson: unused, startLesson: unused,
    acknowledgeReading: unused, completeLesson: unused, listFiles: unused, readFile: unused, writeFile: unused,
    resetWorkspace: unused, runAction: unused, requestAi: unused, getPaneRatio: unused, setPaneRatio: unused,
    ...overrides,
  } as SynaploomApiClient;
}

function Harness({ onProgressChanged = vi.fn() }: { readonly onProgressChanged?: () => void }): ReactNode {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={() => state.setAnswer(answer)}>answer</button>
      <button type="button" onClick={() => void state.saveDraft()}>save</button>
      <button type="button" onClick={() => void state.submit()}>submit</button>
...
describe('useActivityAttempt', () => {
  it('moves through not-started, ready, draft and evaluated states', async () => {
    ...
    render(<AppProviders api={api({ saveActivityDraft: save, submitActivityAttempt: submit })}><Harness onProgressChanged={progress} /></AppProviders>);
    ...
    expect(save).toHaveBeenCalledWith(owner, 'quiz', expect.objectContaining({ answer, revision: 0 }));
    fireEvent.click(screen.getByRole('button', { name: 'submit' }));
    await waitFor(() => expect(screen.getByTestId('state')).toHaveTextContent('evaluated'));
    expect(submit).toHaveBeenCalledWith(owner, 'quiz', expect.objectContaining({ answer, idempotencyKey: expect.any(String) }));
  ...
  it('retains the learner answer after a network error', async () => {
    ...
    render(<AppProviders api={api({ submitActivityAttempt: submit })}><Harness /></AppProviders>);
  ...
  it('does not replace a newer local revision with an older draft response', async () => {
    ...
    const save = vi.fn().mockReturnValue(new Promise<ActivityAttempt>((resolve) => { resolveSave = resolve; }));
    render(<AppProviders api={api({ getCurrentActivityAttempt: () => Promise.resolve(attempt('DRAFT', 3)), saveActivityDraft: save })}><Harness /></AppProviders>);
    await waitFor(() => expect(screen.getByTestId('state')).toHaveTextContent('draft'));
    fireEvent.click(screen.getByRole('button', { name: 'save' }));
