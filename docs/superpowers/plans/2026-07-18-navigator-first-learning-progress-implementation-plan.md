...
- [x] **Step 1:** Add a pure helper that counts required lessons and creates explicit Vietnamese labels.
- [x] **Step 2:** Replace the ambiguous `LessonProgress` widget with the explicit labels in the lesson heading.
- [x] **Step 3:** Keep legacy routes functional by falling back to the flat course lesson list.
- [x] **Step 4:** Commit with `feat: clarify learning progress summary`.
...
- [x] **Step 1:** Remove internal collapsed state and toggle UI from `SynLessonProgress`.
- [x] **Step 2:** Render chapter status, numbered lesson positions, required/optional labels, assessment identity, and accessible state text.
- [x] **Step 3:** Keep locked items focusable with `aria-disabled` and call `onLockedItem` instead of navigating.
- [x] **Step 4:** Update styles for hierarchy density, state markers, and responsive wrapping.
- [x] **Step 5:** Commit with `feat: refine hierarchical course navigator`.
...
- [x] **Step 1:** Add `Nội dung khóa học` trigger with `aria-expanded` and `aria-controls`.
- [x] **Step 2:** Render `SynLessonProgress` only while open.
- [x] **Step 3:** Show an inline `role="alert"` with readable blocking requirement labels after a locked click.
- [x] **Step 4:** Clear stale locked feedback when an unlocked item is selected or the drawer closes.
- [x] **Step 5:** Commit with `feat: add course navigation drawer`.
...
- [x] **Step 2:** Include targeted commands and expected results without running them.
- [x] **Step 3:** Commit with `docs: add navigator-first verification guide`.
- [x] **Step 4:** Create a full source ZIP with Git bundle, log, status, and checksums.
